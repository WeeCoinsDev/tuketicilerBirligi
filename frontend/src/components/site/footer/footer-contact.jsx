...
function ContactItem({ children, icon: Icon }) {
  return (
    <li className="group/contact flex gap-2.5">
      <Icon
        aria-hidden="true"
        className="mt-0.5 size-4 shrink-0 text-secondary transition duration-200 group-hover/contact:-translate-y-px group-hover/contact:text-primary-dark"
        strokeWidth={1.8}
      />
      <span>{children}</span>
    </li>
...
export function FooterContact({ settings, title }) {
  return (
    <div className="rounded-xl border border-line/80 bg-white/72 p-4 shadow-[0_16px_40px_rgba(22,32,51,0.07)] backdrop-blur-sm transition duration-200 hover:-translate-y-0.5 hover:border-primary-dark/25">
