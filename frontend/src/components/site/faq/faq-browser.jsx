...
function FaqItem({
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex h-full flex-col p-5 md:p-7", item.summary && "pr-16 md:pr-24")}>
        {accordion ? (
          <button
            aria-controls={`${item.slug}-answer`}
            aria-expanded={open}
            className="focus-ring text-left"
            onClick={onToggle}
            type="button"
          >
            <h2 className="font-heading text-lg font-semibold leading-snug tracking-tight text-ink md:text-xl md:leading-snug">
              {item.title}
            </h2>
          </button>
        ) : (
        ...
        >
          <div className="overflow-hidden">
            {item.body ? (
              <p className="mt-4 max-w-3xl text-sm leading-7 text-muted md:text-[15px] md:leading-8">{item.body}</p>
            ) : null}
          </div>
        </div>

        <div className="mt-6 flex items-center justify-between gap-3 border-t border-line/80 pt-4">
          <span className="text-xs font-medium text-ink/55">{item.summary}</span>

          {accordion && !open ? (
            <button
              className="focus-ring inline-flex items-center gap-1.5 text-xs font-semibold text-ink/60 transition group-hover:text-secondary"
              onClick={onToggle}
              type="button"
            >
              {showAnswerLabel}
            ...
            <div className="flex items-center gap-3">
              {accordion ? (
                <button
                  className="focus-ring text-xs font-semibold text-ink/50 transition hover:text-ink"
                  onClick={onToggle}
                  type="button"
                >
                  {hideAnswerLabel}
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) => {
        ...
        return `${item.title} ${item.body || ""} ${item.summary || ""}`
  ...
  return (
    <div className="grid gap-8 md:gap-10">
        ...
        <div aria-label={categoriesLabel} className="flex flex-wrap justify-center gap-x-1 gap-y-2">
          {[allLabel, ...categories].map((category, index) => {
            ...
            return (
              <button
                className={cn(
                  "focus-ring rounded-full px-3.5 py-1.5 text-sm font-medium transition",
      ...
      {filteredItems.length ? (
        ...
        <div className="mx-auto w-full max-w-3xl rounded-[24px] border border-dashed border-line bg-white/80 px-6 py-14 text-center text-sm text-muted">
          {emptyText}
        </div>
      )}
    </div>
