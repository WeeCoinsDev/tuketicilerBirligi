...
export function DesktopNavbar({
  ...
  return (
    <nav
      aria-label="Ana menü"
      className={cn("flex h-full w-full items-stretch font-sans text-[13px] font-medium text-nowrap text-ink xl:text-sm", className)}
    >
      <Menu setActive={setActive} linkComponent={linkComponent} className={cn("h-full gap-0 lg:gap-0.5 xl:gap-1.5 2xl:gap-2", menuClassName)}>
        {items.map((entry) => {
