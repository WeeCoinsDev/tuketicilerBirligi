...
export function MenuItem({
  ...
  const triggerClassName = cn(
    "relative z-0 inline-flex items-center gap-1 px-2.5 py-2 text-nowrap transition-colors duration-200 xl:px-3",
    showHighlight ? "text-ink" : "text-ink/80",
    className
  );
      ...
      {hasDropdown ? (
        <ChevronDown
          ...
          className={cn(
            ...
            showDropdown && "rotate-180 opacity-70"
          )}
        />
      ) : null}
    </>
      ...
      {showDropdown ? (
        ...
        >
          <motion.div
            ...
            className={cn(
              "relative w-max overflow-visible rounded-xl border border-line bg-white text-ink shadow-soft",
              dropdownPanelClassName
            )}
          >
